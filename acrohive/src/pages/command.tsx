...
// ─────────────────────────────────────────────
// MOCK AUTH  — replace with your real auth hook
// ─────────────────────────────────────────────
const useAuth = () => ({
...
function AdminDashboard({ userId }: { userId: string }) {
  ...
  const [metrics, setMetrics] = useState({ totalEvents: 0, myEvents: 0, totalRegistrations: 0, attendanceRate: '—' });
    ...
    <div className="bg-[#0d1a2e] border border-cyan-400/20 rounded-xl p-6 flex flex-col max-h-[420px]">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 scrollbar-hide">
        {activities.length === 0 || idle ? (
          <p className="text-gray-500 text-sm text-center mt-6 animate-pulse">● Listening for events…</p>
        ) : (
            ...
            >
              <span className="text-gray-200 leading-snug">
                {act.type === 'registration'
                  ? `👤 ${act.studentName} registered for ${act.eventTitle}`
                  : `✅ ${act.studentName} checked in to ${act.eventTitle} • ${act.method}`}
              </span>
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-800">
          <div>
            <h2 className="text-lg font-bold">{event.title} — Attendees</h2>
              ...
              {filtered.map(a => (
                ...
                >
                  ...
                  <td className="px-6 py-3.5">
                    {a.checkedIn ? (
                      <span className="text-green-400 font-mono text-xs">✅ {a.checkedInAt ? formatDate(a.checkedInAt) : '—'}</span>
                    ) : (
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-[#0d1a2e] border border-cyan-400/20 rounded-xl p-6 space-y-4">
        ...
        >
          <option value="">Select event to generate certificates for…</option>
          {events.map(e => (
        ...
        <div className="flex gap-3">
          ...
          >
            {generating ? <><RefreshCw size={16} className="animate-spin" /> Generating via AI…</> : <><Cpu size={16}/> Generate Certificates</>}
          </button>
          {certs.length > 0 && (
            <button
              onClick={downloadAll}
            ...
            <tbody>
              {registrations.map(r => (
                ...
                >
                  ...
                  <td className="py-3.5">
                    {r.checkedIn
                      ? <span className="text-green-400 text-xs">✅ {r.checkedInAt ? new Date(r.checkedInAt).toLocaleTimeString('en-IN', { hour12: false }) : ''}</span>
                      : <span className="text-gray-500 text-xs">⏳ Not yet</span>}
                  </td>
                </tr>
              ))}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="w-44 h-44 mx-auto bg-white rounded-xl flex items-center justify-center mb-4">
          {/* Real QR would use a library like qrcode.react — showing placeholder SVG */}
